...
export const InboxItem = forwardRef<HTMLButtonElement, Props>(function InboxItem(
  { item, index, onItemClick },
  ref,
) {
  const { params, setParams } = useInboxParams();
  const { data: user } = useUserQuery();
  const { selectedIds, toggleSelection, setLastClickedIndex, clearSelection } = useInboxStore();

  const isNavigationSelected = params.inboxId === item.id || (!params.inboxId && index === 0);
  const isBulkSelected = selectedIds[item.id] === true;
  const isSelectionMode = Object.keys(selectedIds).length > 0;
  const isSelected = isBulkSelected || (!isSelectionMode && isNavigationSelected);
      ...
      <div className="flex w-full flex-col gap-1">
        <div className="flex items-center mb-1">
          <div className="flex items-center gap-2">
            <div className="flex items-center space-x-2 select-text">
              <div className="font-semibold">
                {isProcessing ? <Skeleton className="h-3 w-[120px] mb-1" /> : item.displayName}
              </div>
              {!isProcessing && item.relatedCount !== undefined && item.relatedCount > 0 && (
          ...
          >
            ...
            {!isProcessing && item?.date && formatDate(item.date, user?.dateFormat)}
          </div>
        </div>
        ...
        <div className="flex">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="cursor-help">
                        <FormatAmount amount={item.amount ?? 0} currency={item.currency} />
                      </span>
                    </TooltipTrigger>
                    <TooltipContent className="text-xs px-2 py-1">
                      <div className="flex flex-col gap-0.5">
                        <span>
                          {item.taxType && `${getTaxTypeLabel(item.taxType)} `}
                          <FormatAmount
          ...
          <div className="ml-auto">
            {isProcessing ? <Skeleton className="h-4 w-[60px]" /> : <InboxStatus item={item} />}
          </div>
        </div>
      </div>
    </button>
