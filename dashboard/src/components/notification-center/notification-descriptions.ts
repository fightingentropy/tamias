...
const handleTransactionsCreated: NotificationDescriptionHandler = (metadata, user, t) => {
...
const handleInvoicePaid: NotificationDescriptionHandler = (metadata, user, t) => {
...
const handleInvoiceOverdue: NotificationDescriptionHandler = (metadata, _user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
  return invoiceNumber
...
const handleInvoiceScheduled: NotificationDescriptionHandler = (metadata, user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
...
const handleInvoiceSent: NotificationDescriptionHandler = (metadata, _user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
  const customerName = metadata?.customerName;
...
const handleInvoiceReminderSent: NotificationDescriptionHandler = (metadata, _user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
  const customerName = metadata?.customerName;
  if (invoiceNumber && customerName) {
...
const handleInvoiceCancelled: NotificationDescriptionHandler = (metadata, _user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
  const customerName = metadata?.customerName;
...
const handleInvoiceCreated: NotificationDescriptionHandler = (metadata, user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
  const customerName = metadata?.customerName;
...
const handleInvoiceRefunded: NotificationDescriptionHandler = (metadata, _user, t) => {
  const invoiceNumber = metadata?.invoiceNumber;
  const customerName = metadata?.customerName;
...
const handleRecurringSeriesStarted: NotificationDescriptionHandler = (metadata, _user, t) => {
  const customerName = metadata?.customerName;
  ...
  const frequency = rawFrequency ? getFrequencyShortLabel(rawFrequency) : undefined;

  if (customerName && frequency) {
    if (endType === "after_count" && endCount) {
      return t("notifications.recurring_series_started.with_customer_and_count", {
        customerName,
        frequency,
        count: endCount,
      });
...
const handleRecurringSeriesCompleted: NotificationDescriptionHandler = (metadata, _user, t) => {
  const customerName = metadata?.customerName;
  const totalGenerated = metadata?.totalGenerated;

  if (customerName && totalGenerated) {
    return t("notifications.recurring_series_completed.with_customer_and_count", {
      customerName,
...
const handleRecurringSeriesPaused: NotificationDescriptionHandler = (metadata, _user, t) => {
  const customerName = metadata?.customerName;
  ...
  if (reason === "auto_failure" && failureCount) {
    if (customerName) {
      return t("notifications.recurring_series_paused.auto_failure_with_customer", {
        customerName,
        failureCount,
      });
    }
...
const handleRecurringInvoiceUpcoming: NotificationDescriptionHandler = (metadata, user, t) => {
  ...
  if (count === 1 && invoices?.[0]) {
    ...
    if (invoice.customerName) {
      return t("notifications.recurring_invoice_upcoming.single_with_customer", {
        customerName: invoice.customerName,
      });
    }
  }
...
const handleInboxAutoMatched: NotificationDescriptionHandler = (metadata, user, t) => {
...
const handleInboxNeedsReview: NotificationDescriptionHandler = (metadata, user, t) => {
  const documentName = metadata?.documentName;
  const transactionName = metadata?.transactionName;
  ...
  if (
    ...
    if (matchType === "high_confidence") {
      return t("notifications.inbox_needs_review.cross_currency_high_confidence", {
        documentName,
        transactionName,
        documentAmount: formattedDocAmount,
        transactionAmount: formattedTransAmount,
...
const handleInsightReady: NotificationDescriptionHandler = (metadata, _user, t) => {
...
const handleInboxCrossCurrencyMatched: NotificationDescriptionHandler = (metadata, user, t) => {
  const documentName = metadata?.documentName;
  const transactionName = metadata?.transactionName;
  const documentAmount = metadata?.documentAmount;
  const documentCurrency = metadata?.documentCurrency;
  ...
  if (
    ...
    if (matchType === "high_confidence") {
      return t("notifications.inbox_cross_currency_matched.high_confidence_details", {
        documentName,
        transactionName,
        documentAmount: formattedDocAmount,
  ...
  if (documentName && transactionName) {
    if (matchType === "high_confidence") {
      return t("notifications.inbox_cross_currency_matched.high_confidence_names", {
        documentName,
        transactionName,
      });
    }
