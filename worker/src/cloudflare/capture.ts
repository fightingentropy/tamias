...
async function processSyncInstitutionsMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { SyncInstitutionsProcessor } =
      await import("../processors/institutions/sync-institutions");
    return new SyncInstitutionsProcessor();
  });
}

async function processInboxSyncSchedulerMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { SyncSchedulerProcessor } = await import("../processors/inbox/sync-scheduler");
    return new SyncSchedulerProcessor();
  });
}

async function processNoMatchSchedulerMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { NoMatchSchedulerProcessor } = await import("../processors/inbox/no-match-scheduler");
    return new NoMatchSchedulerProcessor();
  });
}

async function processBatchProcessMatchingMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { BatchProcessMatchingProcessor } =
      await import("../processors/inbox/batch-process-matching");
    return new BatchProcessMatchingProcessor();
  });
}

async function processInitialInboxSetupMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { InitialSetupProcessor } = await import("../processors/inbox/initial-setup");
...
async function processMatchTransactionsBidirectionalMessage(
  ...
  return handleProcessorMessage(message, async () => {
    const { MatchTransactionsBidirectionalProcessor } =
      await import("../processors/inbox/match-transactions-bidirectional");
    return new MatchTransactionsBidirectionalProcessor();
  });
}

async function processAttachmentMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { ProcessAttachmentProcessor } = await import("../processors/inbox/process-attachment");
    return new ProcessAttachmentProcessor();
  });
}

async function processSlackUploadMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { SlackUploadProcessor } = await import("../processors/inbox/slack-upload");
    return new SlackUploadProcessor();
  });
}

async function processWhatsAppUploadMessage(message: Message<CloudflareAsyncMessage>) {
  return handleProcessorMessage(message, async () => {
    const { WhatsAppUploadProcessor } = await import("../processors/inbox/whatsapp-upload");
...
async function processQueueMessage(
  ...
  try {
    ...
    if (body.queueName === "inbox" && body.jobName === "no-match-scheduler") {
    ...
    } else if (body.queueName === "inbox" && body.jobName === "batch-process-matching") {
      result = await processBatchProcessMatchingMessage(message);
    } else if (body.queueName === "inbox" && body.jobName === "match-transactions-bidirectional") {
      result = await processMatchTransactionsBidirectionalMessage(message);
    } else if (body.queueName === "inbox" && body.jobName === "process-attachment") {
    ...
    } else if (body.queueName === "inbox" && body.jobName === "whatsapp-upload") {
      result = await processWhatsAppUploadMessage(message);
    } else if (body.queueName === "inbox-provider" && body.jobName === "initial-setup") {
      ...
      throw new Error(`Missing Cloudflare capture handler for ${body.queueName}:${body.jobName}`);
    }
...
export async function handleCaptureQueueBatch(
  ...
  for (const [index, result] of results.entries()) {
    ...
    logger.error("Cloudflare capture batch message failed outside message handler", {
      queueName: body?.queueName,
      jobName: body?.jobName,
      runId: body?.runId,
