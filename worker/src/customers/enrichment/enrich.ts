...
const enrichmentResearchSystemPrompt = `You are a fast company research agent. Find key company information in ONE search.
...
export async function enrichCustomer(
  ...
  try {
    ...
    const { text: researchText, steps } = await generateText({
      model: google("gemini-3-flash-preview"),
      system: enrichmentResearchSystemPrompt,
      tools: enrichmentResearchTools,
