...
export abstract class BaseProcessor<TData = unknown> {
  ...
  /** Main handler called by the async worker runtime */
  async handle(job: Job<TData>): Promise<unknown> {
    ...
    try {
      ...
      // Ensure result is JSON-serializable so the async runtime can persist it safely
      if (result !== undefined && result !== null) {
        try {
          // Test serialization to ensure it's valid JSON
          const serialized = JSON.stringify(result);
          // Warn on unusually large results before they hit runtime/storage limits
  ...
  /**
   * Update job progress
   * @param job - The async runtime job
