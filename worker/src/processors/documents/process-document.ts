 ...
 */
const CHILD_RUN_POLL_INTERVAL_MS = 500;

async function waitForQueuedRunCompletion(runId: string, timeoutMs: number) {
    ...
    try {
      ...
      if (mimetype === "image/heic") {
        ...
        try {
          ...
          fileData = new Blob([new Uint8Array(image)], {
            type: "image/jpeg",
          });
      ...
      if (processedMimetype.startsWith("image/")) {
        ...
        // Trigger image classification and wait for completion
        ...
        // Use a timestamped requested job ID so manual retries stay distinguishable in logs/status.
        const imageClassificationRun = await enqueue(
          "classify-image",
          {
            fileName,
            teamId,
          ...
          "documents",
          {
            ...
            metadata: {
              internal: true,
              parentRunId: job.runId ?? job.id,
            },
          },
        );

        await waitForQueuedRunCompletion(
          imageClassificationRun.runId,
          TIMEOUTS.CLASSIFICATION_JOB_WAIT,
        );
      ...
      // Trigger document classification and wait for completion
      // This ensures errors propagate and status is properly updated
      // Use CLASSIFICATION_JOB_WAIT timeout to ensure we don't timeout before the child job completes
      // Child job uses AI_CLASSIFICATION (90s), so we need at least that + overhead
      // Use a timestamped requested job ID so manual retries stay distinguishable in logs/status.
      const classificationRun = await enqueue(
        "classify-document",
        {
          content: sample,
          fileName,
      ...
      const classificationJobResult = await waitForQueuedRunCompletion(
        classificationRun.runId,
        TIMEOUTS.CLASSIFICATION_JOB_WAIT,
      );
      ...
      this.logger.info("Document classification job completed", {
        ...
        fileName,
        teamId,
        triggeredRunId: classificationRun.runId,
        triggeredJobName: "classify-document",
        classificationResult: classificationJobResult.result,
