...
export class ClassifyImageProcessor extends BaseProcessor<ClassifyImagePayload> {
  async process(job: Job<ClassifyImagePayload>): Promise<void> {
    ...
    try {
      ...
      this.logger.warn("AI image classification failed, completing with fallback", {
        fileName,
        teamId,
    ...
    if (classificationResult?.title && classificationResult.title.trim().length > 0) {
      ...
      this.logger.warn("Image classification returned null or empty title - generating fallback", {
        fileName,
        pathTokens,
        teamId,
