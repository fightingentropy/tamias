...
import { getReports, getSpending, getSpendingForPeriod } from "@tamias/app-data/queries";
...
type TransactionRecord = Awaited<ReturnType<typeof getTransactionsPage>>["data"][number];
...
function shouldIncludeTransaction(transaction: TransactionRecord) {
  ...
  if (transaction.amount > 0) {
    if (!transaction.categorySlug || !revenueCategories.includes(transaction.categorySlug)) {
      return false;
    }
...
function getTransactionAmount(transaction: TransactionRecord, targetCurrency: string) {
  if (transaction.baseCurrency === targetCurrency && transaction.baseAmount != null) {
...
async function getAllTransactionsForRange(options: { teamId: string; from: string; to: string }) {
...
function buildBreakdownTransactions(options: {
  ...
  const totalExpenses = Math.abs(
    transactionAmounts.filter((amount) => amount < 0).reduce((sum, amount) => sum + amount, 0),
  );
    ...
    .map((transaction, index) => {
      ...
      const formattedAmount =
        formatAmount({
          ...
          currency: transaction.baseCurrency || transaction.currency || options.targetCurrency,
      ...
      return {
        ...
        percentage: totalForPercentage > 0 ? (Math.abs(amount) / totalForPercentage) * 100 : 0,
      };
    })
...
export async function getMetricsBreakdownPeriodData(
  ...
  const [revenueResult, spendingCategories, periodSummary, profitResult, allTransactions] =
    await Promise.all([
      getReports(db, {
        teamId: options.teamId,
        from: options.from,
...
export function aggregateMonthlyBreakdownResults(options: {
  ...
  const categoryMap = new Map<string, { name: string; amount: number; percentage: number }>();
  ...
  const categories = Array.from(categoryMap.values())
    .map((category) => ({
      ...category,
      percentage:
        options.summary.expenses > 0 ? (category.amount / options.summary.expenses) * 100 : 0,
    }))
  ...
  const transactions = Array.from(transactionMap.values())
    .map((transaction) => {
      ...
      return {
        ...
        percentage:
          totalForPercentage > 0 ? (Math.abs(transaction.amount) / totalForPercentage) * 100 : 0,
      };
    })
    .sort((left, right) => right.percentage - left.percentage)
    .slice(0, 5);
