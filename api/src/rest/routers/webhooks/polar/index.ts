    ...
    try {
      switch (event.type) {
          ...
          try {
            ...
            if (owner?.email && teamHasData) {
              await startCloudflareWorkflow(
                ...
                {
                  publicTeamId: teamId,
                  instanceId: `cancellation-email-${teamId}`,
                },
              );
            }
          } catch (err) {
        ...
        case "subscription.revoked": {
          ...
          // Polar can emit subscription.revoked with status === "past_due" even though
          // there is also a dedicated subscription.past_due event.
