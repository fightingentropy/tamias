...
app.openapi(
  createRoute({
    ...
    description: "Generates OAuth install URL for Fortnox integration. Requires authentication.",
    tags: ["Integrations"],
    responses: {
      200: {
  ...
  async (c) => {
    ...
    try {
      const provider = await getAccountingProvider("fortnox");
      const url = await provider.buildConsentUrl(state);
      return c.json({ url });
    } catch (error) {
      throw new HTTPException(500, {
        message: error instanceof Error ? error.message : "Fortnox OAuth configuration missing",
      });
    }
  },
);
