...
app.openapi(
  ...
  async (c) => {
    ...
    if (error || !code) {
      ...
      return c.redirect(
        buildErrorRedirect(dashboardUrl, errorCode, "outlook", source, "/inbox", redirectPath),
        302,
      );
    }
    ...
    try {
      const connector = new InboxConnector("outlook");
      ...
      await enqueue(
        ...
        "inbox-provider",
        {
          publicTeamId: parsedState.teamId,
        },
      );

      // Redirect based on source
      return c.redirect(
        buildSuccessRedirect(dashboardUrl, "outlook", parsedState.source, "/inbox", redirectPath),
        302,
      );
    } catch (err) {
