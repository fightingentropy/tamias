...
graph TB
    ...
    subgraph storage [Convex Storage]
        Bucket[(vault bucket)]
    ...
    subgraph db [Database]
        ...
        Tags[(document tag store)]
        Embeddings[(document tag embeddings)]
    end
...
- Safe to retry - deterministic requested job IDs prevent duplicate processing
...
| [`apps/worker/src/cloudflare/index.ts`](../apps/worker/src/cloudflare/index.ts) | Queue routing, workflows, schedules, and runtime dispatch |
...
- Storage registration retry
- User clicking retry rapidly
- Network issues causing duplicate API calls

Deterministic IDs (`process-doc:${teamId}:${path}`) ensure the async runtime can reject or ignore duplicate processing attempts consistently.
