...
sequenceDiagram
    ...
    Cron->>Queue: Every hour
    Queue->>Scheduler: Process job
    ...
    loop For each due series
        ...
        alt Already exists
            ...
            Scheduler->>Queue: Queue generate-invoice
            Queue->>Generator: Process
            Generator->>Generator: Generate PDF
            Generator->>Queue: Queue send-invoice-email
            Queue->>Email: Process
...
1. **Scheduler Level**: a single Cloudflare cron trigger owns recurring dispatch
...
| [`packages/app-data/src/queries/invoice-recurring.ts`](../packages/app-data/src/queries/invoice-recurring.ts) | App-data queries (CRUD, state transitions) |
| [`packages/invoice/src/utils/server-recurring.ts`](../packages/invoice/src/utils/server-recurring.ts) | Server-side recurring date utilities |
...
```typescript
import { localDateToUTCMidnight } from "@tamias/invoice/recurring";
