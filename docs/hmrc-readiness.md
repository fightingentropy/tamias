...
Tamias currently uses password authentication and has no installed per-device licence key. Explain the missing MFA/licence values to HMRC. Ask HMRC about any remaining platform restrictions; do not assume an exemption. The prepared correspondence is in `docs/security/hmrc-header-enquiry.md`.
