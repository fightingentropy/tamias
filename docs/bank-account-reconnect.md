...
sequenceDiagram
    ...
    participant Job as Async Worker Job
    ...
    Frontend->>Job: enqueue("reconnect-connection")
    Job-->>Frontend: Return runId
    ...
    Frontend->>Frontend: Subscribe to async run status via Convex
...
Sensitive fields are encrypted at rest using `@tamias/encryption`:
- `iban` - Encrypted
...
| `apps/worker/src/processors/transactions/reconnect-connection.ts` | Main reconnect job - fetches accounts, matches, updates IDs |
| `apps/worker/src/processors/transactions/sync-connection.ts` | Follow-up transaction sync after reconnect |
