...
This dual approach ensures data consistency while the new runway metrics settle into place.
...
import { CASH_ACCOUNT_TYPES, DEBT_ACCOUNT_TYPES } from "@tamias/banking/account";
...
| `packages/app-data/src/queries/reports.ts` | `getRunway()`, `getBurnRate()`, `getBalanceSheet()`, `getCashFlow()` |
| `packages/app-data/src/queries/bank-accounts.ts` | `getNetPosition()`, `getCashBalance()` |
