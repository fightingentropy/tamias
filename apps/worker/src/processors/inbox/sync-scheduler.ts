...
export class SyncSchedulerProcessor extends BaseProcessor<InboxProviderSyncAccountPayload> {
  async process(job: Job<InboxProviderSyncAccountPayload>): Promise<{
    ...
    const accountRow = await getInboxAccountInfo({ id: inboxAccountId });

    if (!accountRow) {
      throw new Error("Account not found");
    }

    const connector = new InboxConnector(accountRow.provider);
      ...
      if (uploadedAttachments.length > 0) {
        ...
        await Promise.all(
          uploadedAttachments.map((attachment) =>
            enqueue("process-attachment", attachment, "inbox"),
          ),
        );
