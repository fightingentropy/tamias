...
export function getPasswordAuthErrorMessage(
  ...
  if (NETWORK_ERROR_PATTERN.test(message)) {
    return "Could not reach Convex (password auth uses it). Run the full stack with `bun run dev` from the repo root, put CONVEX_URL in apps/dashboard/.env.local, and run Convex dev (`bunx convex dev` in apps/dashboard) if you use a dev deployment.";
  }
