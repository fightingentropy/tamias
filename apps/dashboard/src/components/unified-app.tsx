...
import { CompaniesHouseAppPanel } from "./companies-house-app-panel";
...
> = {
  ...
  "companies-house": {
    endpoint: "/apps/companies-house/install-url",
    queryKey: "apps",
  },
  "stripe-payments": {
...
export function UnifiedAppComponent({ app }: UnifiedAppProps) {
  ...
  const handleDisconnect = () => {
    // Gmail and Outlook use inbox account connections
  ...
  const handleOnInitialize = async () => {
    ...
    try {
      ...
      // Handle apps with installUrl by opening their setup page.
      if (app.installUrl) {
                  ...
                  {app.id === "companies-house" && (
                    <AccordionItem value="operations" className="border-none">
                      ...
                      <AccordionContent className="text-[#878787] text-sm">
                        <CompaniesHouseAppPanel installed={Boolean(app.installed)} />
                      </AccordionContent>
                    </AccordionItem>
                  )}
