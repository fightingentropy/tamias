...
export function PdfThumbnail({ url, cacheKey, width = 60, height }: Props) {
  ...
  useEffect(() => {
    if (thumbnailSrc || !reactPdf) return;
  ...
  }, [reactPdf, thumbnailSrc]);

  // Render PDF when visible
  useEffect(() => {
    if (!isVisible || !reactPdf || thumbnailSrc || error) return;
  ...
  }, [cacheKey, reactPdf, isVisible, thumbnailSrc, error, width]);
