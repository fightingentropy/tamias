...
export class TellerApi {
   ...
   * Uses the Worker-bound TELLER_MTLS_CERTIFICATE fetcher for client auth.
   */
  async #fetch(url: string, init?: RequestInit): Promise<Response> {
    const tellerMtlsFetcher = getBankingRuntime().tellerMtlsFetcher;

    if (!tellerMtlsFetcher) {
      throw new Error(
        "Teller mTLS not configured. Bind TELLER_MTLS_CERTIFICATE in the Cloudflare runtime.",
      );
    }

    return tellerMtlsFetcher.fetch(url, init);
  }
