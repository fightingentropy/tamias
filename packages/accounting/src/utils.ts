 ...
 *
 * @param transactionId - The Tamias transaction ID
 * @param jobId - The async run/job identifier (unique per export session)
 ...
 * // Returns: "tamias-attachment-abc-123-receipt.pdf"
 */
export function generateAttachmentIdempotencyKey(
  providerTransactionId: string,
  fileName: string,
): string {
  return `tamias-attachment-${providerTransactionId}-${fileName}`;
}
