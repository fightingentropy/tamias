...
import { ensureFileExtension, PROVIDER_ATTACHMENT_CONFIG, resolveMimeType } from "./utils";

describe("resolveMimeType", () => {
  // Create test buffers with proper magic bytes
  const createPdfBuffer = () => Buffer.from("%PDF-1.4 test content");
  const createJpegBuffer = () => Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10]);
  const createPngBuffer = () => Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
  ...
  describe("stored type resolution", () => {
    ...
    test("returns stored image/jpeg if supported", () => {
      const result = resolveMimeType("image/jpeg", "photo.jpg", createJpegBuffer(), "fortnox");
  ...
  describe("extension inference", () => {
    ...
    test("infers jpeg from .jpg extension", () => {
      const result = resolveMimeType(null, "photo.jpg", createJpegBuffer(), "fortnox");
      expect(result.mimeType).toBe("image/jpeg");
      expect(result.source).toBe("extension");
    });
  ...
  describe("buffer magic byte detection", () => {
    test("detects PDF from magic bytes", () => {
      const result = resolveMimeType(null, null, createPdfBuffer(), "quickbooks");
      expect(result.mimeType).toBe("application/pdf");
    ...
    test("detects GIF from magic bytes (GIF89a)", () => {
      const result = resolveMimeType(null, null, createGifBuffer(), "quickbooks");
      expect(result.mimeType).toBe("image/gif");
      expect(result.source).toBe("buffer");
    });

    test("detects GIF from magic bytes (GIF87a)", () => {
      const result = resolveMimeType(null, null, Buffer.from("GIF87a test"), "quickbooks");
      expect(result.mimeType).toBe("image/gif");
      expect(result.source).toBe("buffer");
    });

    test("detects TIFF little-endian from magic bytes", () => {
      const result = resolveMimeType(null, null, createTiffLeBuffer(), "quickbooks");
      expect(result.mimeType).toBe("image/tiff");
      expect(result.source).toBe("buffer");
    });

    test("detects TIFF big-endian from magic bytes", () => {
      const result = resolveMimeType(null, null, createTiffBeBuffer(), "quickbooks");
      expect(result.mimeType).toBe("image/tiff");
      expect(result.source).toBe("buffer");
    });

    test("detects BMP from magic bytes", () => {
      const result = resolveMimeType(null, null, createBmpBuffer(), "quickbooks");
    ...
    test("detects WebP from magic bytes", () => {
      ...
      const result = resolveMimeType(null, null, createWebpBuffer(), "quickbooks");
  ...
  describe("unsupported type handling", () => {
    ...
    test("returns error when type cannot be determined", () => {
      const result = resolveMimeType(null, null, createUnknownBuffer(), "quickbooks");
      expect(result.mimeType).toBe(null);
      expect(result.source).toBe("failed");
  ...
  describe("provider-specific supported types", () => {
    test("fortnox supports PDF, JPEG, PNG only", () => {
      // Supported
      expect(resolveMimeType("application/pdf", null, createPdfBuffer(), "fortnox").mimeType).toBe(
        "application/pdf",
      );
      expect(resolveMimeType("image/jpeg", null, createJpegBuffer(), "fortnox").mimeType).toBe(
        "image/jpeg",
      );
      expect(resolveMimeType("image/png", null, createPngBuffer(), "fortnox").mimeType).toBe(
        "image/png",
      );
    ...
    test("quickbooks supports PDF, JPEG, PNG, GIF, TIFF, BMP", () => {
      // All supported
      expect(
        resolveMimeType("application/pdf", null, createPdfBuffer(), "quickbooks").mimeType,
      ).toBe("application/pdf");
      expect(resolveMimeType("image/jpeg", null, createJpegBuffer(), "quickbooks").mimeType).toBe(
        "image/jpeg",
      );
      expect(resolveMimeType("image/png", null, createPngBuffer(), "quickbooks").mimeType).toBe(
        "image/png",
      );
      expect(resolveMimeType("image/gif", null, createGifBuffer(), "quickbooks").mimeType).toBe(
        "image/gif",
      );
      expect(resolveMimeType("image/tiff", null, createTiffLeBuffer(), "quickbooks").mimeType).toBe(
        "image/tiff",
      );
      expect(resolveMimeType("image/bmp", null, createBmpBuffer(), "quickbooks").mimeType).toBe(
        "image/bmp",
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    test("prioritizes stored type over extension", () => {
      ...
      const result = resolveMimeType("image/png", "file.jpg", createPngBuffer(), "fortnox");
      expect(result.mimeType).toBe("image/png");
      expect(result.source).toBe("stored");
    });
    ...
    test("handles case-insensitive extensions", () => {
      const result = resolveMimeType(null, "RECEIPT.PDF", createPdfBuffer(), "fortnox");
      expect(result.mimeType).toBe("application/pdf");
      expect(result.source).toBe("extension");
    });
  });
...
describe("PROVIDER_ATTACHMENT_CONFIG", () => {
  ...
  test("quickbooks max size is 20 MB", () => {
    expect(PROVIDER_ATTACHMENT_CONFIG.quickbooks.maxSizeBytes).toBe(20 * 1024 * 1024);
  });
});

describe("ensureFileExtension", () => {
  test("returns unchanged if filename has valid extension", () => {
    expect(ensureFileExtension("invoice.pdf", "application/pdf")).toBe("invoice.pdf");
    expect(ensureFileExtension("receipt.jpg", "image/jpeg")).toBe("receipt.jpg");
    expect(ensureFileExtension("scan.png", "image/png")).toBe("scan.png");
  });

  test("adds extension based on MIME type", () => {
    expect(ensureFileExtension("invoice", "application/pdf")).toBe("invoice.pdf");
  ...
  test("removes trailing dots before adding extension", () => {
    expect(ensureFileExtension("vercel-inc.", "application/pdf")).toBe("vercel-inc.pdf");
  ...
  test("defaults to .pdf for unknown MIME types", () => {
    ...
    expect(ensureFileExtension("document", "application/octet-stream")).toBe("document.pdf");
  });
  ...
  test("preserves existing valid extensions (2-5 chars)", () => {
    expect(ensureFileExtension("file.docx", "application/pdf")).toBe("file.docx");
