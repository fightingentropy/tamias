import { getRequestedSourceTypes, sourceCanMatchFilters } from "../helpers";
...
type SearchCandidateLoader = (params: SearchCandidateLoadParams) => Promise<SearchCandidate[]>;

const semanticSearchCandidateLoaders: Record<SearchSourceType, SearchCandidateLoader> = {
...
export async function loadSearchCandidates(params: SearchCandidateLoadParams) {
  const requestedSourceTypes = getRequestedSourceTypes(params.types).filter((sourceType) =>
    sourceCanMatchFilters(sourceType, params),
  );
  const loads = await Promise.all(
    requestedSourceTypes.map((sourceType) => semanticSearchCandidateLoaders[sourceType](params)),
  );
