import { calculateNextScheduledDate } from "@tamias/invoice/server-recurring";
...
export async function updateInvoiceRecurring(
  ...
  const endConditionsChanged =
    params.endType !== undefined || params.endDate !== undefined || params.endCount !== undefined;
  ...
  if (current && endConditionsChanged) {
    ...
    const mergedEndDate = params.endDate !== undefined ? params.endDate : current.endDate;
    const mergedEndCount = params.endCount !== undefined ? params.endCount : current.endCount;
    ...
    nextScheduledAt = calculateNextScheduledDate(
      {
        ...
        frequencyDay:
          params.frequencyDay !== undefined ? params.frequencyDay : current.frequencyDay,
        frequencyWeek:
          params.frequencyWeek !== undefined ? params.frequencyWeek : current.frequencyWeek,
        frequencyInterval:
          params.frequencyInterval !== undefined
  ...
  return upsertProjectedInvoiceRecurringRecord({
    ...
    endType: hasOwnKey(updateData, "endType")
      ...
      : existing.endType,
    endDate: hasOwnKey(updateData, "endDate") ? (updateData.endDate ?? null) : existing.endDate,
    endCount: hasOwnKey(updateData, "endCount") ? (updateData.endCount ?? null) : existing.endCount,
    ...
    amount: hasOwnKey(updateData, "amount") ? (updateData.amount ?? null) : existing.amount,
    currency: hasOwnKey(updateData, "currency") ? (updateData.currency ?? null) : existing.currency,
    lineItems: hasOwnKey(updateData, "lineItems")
      ? (updateData.lineItems ?? null)
      : existing.lineItems,
    template: hasOwnKey(updateData, "template") ? (updateData.template ?? null) : existing.template,
    ...
    discount: hasOwnKey(updateData, "discount") ? (updateData.discount ?? null) : existing.discount,
    subtotal: hasOwnKey(updateData, "subtotal") ? (updateData.subtotal ?? null) : existing.subtotal,
    topBlock: hasOwnKey(updateData, "topBlock") ? (updateData.topBlock ?? null) : existing.topBlock,
