...
export const SlashMenu = forwardRef<SlashMenuRef, SlashMenuProps>(({ items, command }, ref) => {
  ...
  }, [activeSubmenu, selectedIndex, submenuSelectedIndex, selectItem, selectSubmenuItem]);
      ...
      <div className="z-50 min-w-[180px] overflow-hidden border border-border bg-background p-1 text-popover-foreground shadow-md">
        {items.map((item, index) => (
          ...
          >
            ...
            {item.hasSubmenu && <span className="ml-2 text-muted-foreground">›</span>}
          </button>
        ))}
      </div>

      {/* Submenu */}
      {activeSubmenu && currentItem?.submenuItems && currentItem.submenuItems.length > 0 && (
