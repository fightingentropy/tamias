 ...
 * - Actions: Derived directly from slots for speed and consistency
 */
 ...
 * 3. Actions derived from slots without an LLM call
 */
export class ContentGenerator {
  private model: string;

  constructor(options: ContentGeneratorOptions = {}) {
    // Default to the cheaper GPT-5 tier for generated insight copy.
  ...
  async generate(
    ...
    try {
      ...
      // 2. Generate title and summary in parallel
      const [title, summary] = await Promise.all([
        this.generateTitle(slots),
        this.generateSummary(slots),
      ]);

      // 3. Generate story while deriving deterministic actions from the same slots
      const [story, actions] = await Promise.all([
        this.generateStory(slots),
        Promise.resolve(deriveActions(slots)),
      ]);
