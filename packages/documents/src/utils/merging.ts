...
import { calculateQualityScore, calculateReceiptQualityScore } from "./validation";
...
export function mergeInvoiceResults(
  ...
  const primaryConf =
    primaryConfidence ?? calculateExtractionConfidence(primary, calculateQualityScore(primary));
  const secondaryConf =
    secondaryConfidence ??
    ...
    if (
      ...
      if (Math.abs(primaryConf - secondaryConf) < 0.1) {
        if (invoiceMergeRules[field]) {
          merged[field] = invoiceMergeRules[field](primaryValue, secondaryValue);
        } else {
        ...
        merged[field] = primaryConf >= secondaryConf ? primaryValue : secondaryValue;
      }
    } else {
...
export function mergeReceiptResults(
  ...
  const primaryConf =
    primaryConfidence ??
    calculateReceiptExtractionConfidence(primary, calculateReceiptQualityScore(primary));
  const secondaryConf =
    secondaryConfidence ??
    ...
    if (
      ...
      if (Math.abs(primaryConf - secondaryConf) < 0.1) {
        if (receiptMergeRules[field]) {
          merged[field] = receiptMergeRules[field](primaryValue, secondaryValue);
        } else {
          // Prefer primary if confidences are similar
          merged[field] = primaryValue;
        }
